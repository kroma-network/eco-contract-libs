...
describe("Echidna", function () {
  async function HHKromaBridgeFixture() {
    ...
    const bridgeL1Factory =
      await hre.ethers.getContractFactory("HHL1KromaBridge");
    const bridgeL2Factory =
      await hre.ethers.getContractFactory("HHL2KromaBridge");
    ...
    const tokenL1Factory = await hre.ethers.getContractFactory(
      "EcoERC20Upgradeable",
    );
    const tokenL2Factory = await hre.ethers.getContractFactory(
      "ERC20L2BridgedUpgradeable",
    );
    const tokenL1 = await tokenL1Factory.deploy();
    const tokenL2 = await tokenL2Factory.deploy();

    await expect(tokenL1.initEcoERC20(owner, "tokenL1", "TL1", 18)).not
      .reverted;
    await expect(
      tokenL2.initERC20L2Bridged(
        owner,
        "tokenL2",
        "TL2",
        18,
  ...
  describe("Basic", function () {
    it("Bridge", async function () {
      const { owner, bridgeL1, bridgeL2, tokenL1, tokenL2 } =
        await loadFixture(HHKromaBridgeFixture);
      ...
      await expect(bridgeL1.bridgeERC20(tokenL1, tokenL2, amount, 200000, "0x"))
        .not.reverted;
      expect(await tokenL2.balanceOf(owner)).eq(amount);

      await expect(bridgeL2.bridgeERC20(tokenL2, tokenL1, amount, 200000, "0x"))
        .not.reverted;
      expect(await tokenL2.balanceOf(owner)).eq(0);
    });
    ...
    it("BridgeTo", async function () {
      ...
      await expect(
        bridgeL1.bridgeERC20To(tokenL1, tokenL2, owner, amount, 200000, "0x"),
      ).not.reverted;
      expect(await tokenL2.balanceOf(owner)).eq(amount);

      await expect(
        bridgeL2.bridgeERC20To(tokenL2, tokenL1, owner, amount, 200000, "0x"),
      ).not.reverted;
      expect(await tokenL2.balanceOf(owner)).eq(0);
    });

    it("Bridge Token Coverage(revert)", async function () {
      const { owner, users, tokenL1, tokenL2, bridgeL2 } =
        await loadFixture(HHKromaBridgeFixture);

      const amount = ethers.parseEther("1");
      await tokenL2.mint(users[0], amount);
      await expect(
        tokenL2.connect(users[0])["burn(address,uint256)"](users[0], amount),
      ).reverted;
      ...
      await expect(
        tokenL2InitRevert.initERC20L2Bridged(
          users[0],
          "tokenL2",
          "TL2",
          18,
    ...
    it("Bridge Coverage(revert)", async function () {
      const { owner, users, bridgeL1, bridgeL2 } =
        await loadFixture(HHKromaBridgeFixture);
      await expect(bridgeL1.bridgeETHTo(ZeroAddress, 0, "0x")).reverted;
      await expect(bridgeL1.bridgeETH(0, "0x")).reverted;
      await expect(
        bridgeL1.finalizeBridgeETH(ZeroAddress, ZeroAddress, 0, "0x"),
      ).reverted;
      await expect(bridgeL1.connect(users[0]).setRemoteBridge(ZeroAddress))
        .reverted;
      await expect(
        bridgeL1
          .connect(users[0])
      ...
      await expect(
        bridgeL2.finalizeBridgeETH(ZeroAddress, ZeroAddress, 0, "0x"),
      ).reverted;
      await expect(bridgeL2.connect(users[0]).setRemoteBridge(ZeroAddress))
        .reverted;
      await expect(
        bridgeL2
          .connect(users[0])

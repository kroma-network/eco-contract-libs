...
describe("ProxyWithDeployAdmin Test", function () {
  ...
  describe("Deployment", function () {
    it("ProxyWithDeployAdmin check", async function () {
      const { erc20Logic, proxy } = await loadFixture(
    ...
    it("UUPS upgrade", async function () {
      const { owner, inst, users } = await loadFixture(
        fixtureProxyWithDeployAdminConfig,
      );
      ...
      await expect(
        inst.connect(users[0]).upgradeToAndCall(BridgedEcoERC20Logic, "0x"),
      ).reverted;
      await expect(inst.upgradeToAndCall(BridgedEcoERC20Logic, "0x")).not
        .reverted;

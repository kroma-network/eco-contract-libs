...
describe("ProxyWithDeployAdmin Test", function () {
  ...
  async function fixtureProxyWithDeployAdminConfig() {
    ...
    const EcoTUPDeployAdmin = await hre.ethers.getContractFactory("EcoTUPDeployAdmin");
    ...
    return { erc20Logic, proxyAdmin, proxy, inst, owner, users, EcoTUPDeployAdmin };
  }

  describe("Deployment", function () {
    it("ProxyWithDeployAdminAdmin check", async function () {
      const { proxyAdmin, owner } = await loadFixture(fixtureProxyWithDeployAdminConfig);
    ...
    it("ProxyWithDeployAdmin check", async function () {
      const { erc20Logic, proxyAdmin, proxy } = await loadFixture(fixtureProxyWithDeployAdminConfig);
    ...
    it("proxy admin check: upgrade, call, calls, delcall, get/setSlot", async function () {
      const { owner, proxyAdmin, proxy, inst, users, erc20Logic } = await loadFixture(fixtureProxyWithDeployAdminConfig);
      ...
      await expect(hackedProxyWithDeployAdminAdmin.connect(owner).functionCallWithValue(
        proxy,
        upgradeCalldata,
        0
      )).reverted;

      await expect(hackedProxyWithDeployAdminAdmin.functionCallWithValue(
        proxy,
        upgradeCalldata,
        0
      )).not.reverted;
      ...
      const delegateCalldata = hackedProxyWithDeployAdminAdmin.interface.encodeFunctionData("transferOwnership", [owner.address]);
      await expect(hackedProxyWithDeployAdminAdmin.connect(owner).functionDelegateCall(hackedProxyWithDeployAdminAdmin, delegateCalldata)).reverted;
      await expect(hackedProxyWithDeployAdminAdmin.functionDelegateCall(hackedProxyWithDeployAdminAdmin, delegateCalldata)).not.reverted;

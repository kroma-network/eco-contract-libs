...
describe("ProxyWithAdminLogic Test", function () {
  ...
  describe("Deployment", function () {
    it("ProxyWithAdminLogicAdmin check", async function () {
      const { proxyAdminLogic, owner } = await loadFixture(fixtureProxyWithAdminLogicConfig);
    ...
    it("ProxyWithAdminLogic check", async function () {
      const { erc20Logic, proxyAdmin, proxy } = await loadFixture(fixtureProxyWithAdminLogicConfig);
    ...
    it("proxy admin check: upgrade, call, calls, delcall, get/setSlot", async function () {
      const { owner, proxyAdmin, proxy, inst, users, erc20Logic } = await loadFixture(fixtureProxyWithAdminLogicConfig);
      ...
      await expect(hackedProxyWithAdminLogicAdmin.connect(owner).functionCallWithValue(
        proxy,
        upgradeCalldata,
        0
      )).reverted;

      await expect(hackedProxyWithAdminLogicAdmin.functionCallWithValue(
        proxy,
        upgradeCalldata,
        0
      )).not.reverted;
      ...
      const delegateCalldata = hackedProxyWithAdminLogicAdmin.interface.encodeFunctionData("transferOwnership", [owner.address]);
      await expect(hackedProxyWithAdminLogicAdmin.connect(owner).functionDelegateCall(hackedProxyWithAdminLogicAdmin, delegateCalldata)).reverted;
      await expect(hackedProxyWithAdminLogicAdmin.functionDelegateCall(hackedProxyWithAdminLogicAdmin, delegateCalldata)).not.reverted;

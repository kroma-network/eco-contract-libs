...
describe("Script Library Test(Upgradeable ERC20)", function () {
  ...
  async function ERC20_Upgrade_Fixture() {
    ...
    const erc20 = await new ProxiedInstance(
      "erc20",
      EcoERC20Upgradeable__factory,
      owner,
    );
    await erc20.deployWithInputBuilder(async () => {
      return erc20.logic.interface.encodeFunctionData("initEcoERC20", [
        owner.address,
        name,
        symbol,
        decimals,
  ...
  describe("Deployment", function () {
    it("Should set the right owner", async function () {
      ...
      await expect(erc20.inst.initEcoERC20(owner, name, symbol, decimals))
        .reverted;
    });
  ...
  describe("ERC20 Upgrade", function () {
    describe("Mint", function () {
      ...
      it("Shouldn't fail mint with the right role access account", async function () {
        ...
        await expect(
          erc20.inst.grantSelectorRole(getSelector(erc20.inst.mint), users[0]),
        ).not.reverted;
        ...
        await expect(
          erc20.inst.revokeSelectorRole(getSelector(erc20.inst.mint), users[0]),
        ).not.reverted;
        await expect(user_connected_nft.mint(users[0], amount)).reverted;
      });
    });
    ...
    describe("Transfer", function () {
      it("Should revert with the right error if mint called from another account", async function () {
        ...
        await erc20.inst
          .connect(users[0])
          .approve(owner, hre.ethers.MaxUint256);
        await expect(
          erc20.inst.transferFrom(
            users[0],
            users[1],
            await erc20.inst.balanceOf(users[0]),
          ),
        ).not.reverted;
        await expect(erc20.inst.transferFrom(users[0], users[1], amount))
          .reverted;
        await expect(
          erc20.inst
            .connect(users[1])
            .transferFrom(
              users[1],
              users[0],
              await erc20.inst.balanceOf(users[1]),
            ),
        ).reverted;
        await erc20.inst
          .connect(users[1])
          .approve(owner, hre.ethers.MaxUint256);
        await expect(
          erc20.inst.burnFrom(users[1], await erc20.inst.balanceOf(users[1])),
        ).not.reverted;
      });
    });
  });

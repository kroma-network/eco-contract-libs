...
describe("ERC1155 Coverage", function () {
  ...
  describe("ERC1155Pausable Coverage(Revert)", function () {
    it("ERC1155Pausable onlyInitializing: revert NotInitializing", async function () {
      ...
      const erc1155PausableFactory = await ethers.getContractFactory(
        "HHERC1155PausableUpgradeable",
      );
  ...
  describe("ERC1155URIStorage Coverage(Revert)", function () {
    it("ERC1155Pausable onlyInitializing: revert NotInitializing", async function () {
      const [owner, ...users] = await ethers.getSigners();
      const erc1155URIStorageFactory = await ethers.getContractFactory(
        "HHEcoERC1155URIControl",
      );
  ...
  describe("ERC1155 Receiver Coverage", function () {
    it("ERC1155 Receiver Receiving Token", async function () {
      ...
      const erc1155ReceiverFactory =
        await ethers.getContractFactory("HHERC1155Receiver");
    ...
    it("ERC1155 Receiver Simple View", async function () {
      ...
      const erc1155ReceiverInterfaceId =
        await erc1155Receiver.IERC1155ReceiverInterfaceId();

      expect(await erc1155Receiver.supportsInterface(mockInterfaceId)).eq(
        false,
      );
      expect(
        await erc1155Receiver.supportsInterface(erc1155ReceiverInterfaceId),
      ).eq(true);
      ...
      expect(
        await erc1155Receiver.onERC1155BatchReceived(
          ZeroAddress,
          ZeroAddress,
          [],
          [],

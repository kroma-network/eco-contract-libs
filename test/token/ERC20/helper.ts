...
export async function ERC20_Rebase_Native_Fixture() {
  ...
  await erc20Rebased.initEcoERC20Rebase(
    ZeroAddress,
...
export async function ERC20_Rebase_Token_Base_Fixture() {
  ...
  const ERC20underlying = await hre.ethers.getContractFactory(
    "EcoERC20Upgradeable",
  );
  const erc20Underlying = await ERC20underlying.deploy();
  await expect(
    erc20Underlying.initEcoERC20(
      owner,
  ...
  const ERC20Rebased = await hre.ethers.getContractFactory(
    "EcoERC20RebasedWithToken",
  );
...
export async function ERC20_Rebase_Token_Fixture() {
  const { erc20Underlying, erc20Rebased, owner, users } = await loadFixture(
  ...
  await Promise.all(
    users
      ...
      .map(
        (user) =>
          expect(erc20Underlying.mint(user, 10n * unitAmount)).not.reverted,
      ),
  );
  await Promise.all(
    users
      ...
      .map(
        (user) =>
          expect(
            erc20Underlying.connect(user).approve(erc20Rebased, MaxUint256),
          ).not.reverted,
      ),
  );
...
export async function ERC4626_with_Token_Fixture() {
  ...
  const ERC4626Token = await hre.ethers.getContractFactory(
    "EcoERC4626Upgradeable",
  );
  const erc4626Token = await ERC4626Token.deploy();
  await expect(
    erc4626Token.initEcoERC4626(erc20, erc4626TokenName, erc4626TokenSymbol),
  ).not.reverted;
  ...
  await Promise.all(
    users
      .slice(0, 4)
      .map((user) => expect(erc20.mint(user, 10n * unitAmount)).not.reverted),
  );
  await Promise.all(
    users
      .slice(0, 4)
      .map(
        (user) =>
          expect(erc20.connect(user).approve(erc4626Token, MaxUint256)).not
            .reverted,
      ),
  );

...
type EcoContractFactory<CT extends BaseContract> =
  ContractFactoryTypeSupporter<CT> & ContractFactory;
...
export interface EcoProxyPropertiesWithFactory<CT extends BaseContract>
  extends EcoProxyBaseProperties<CT> {
  ecoLogicFactory: EcoContractFactory<CT>;
}

export type EcoProxiedInstance<CT extends BaseContract> = CT &
  EcoProxyBaseProperties<CT>;
export type EcoProxiedInstanceWithFactory<CT extends BaseContract> = CT &
  EcoProxyPropertiesWithFactory<CT>;
...
export class ProxyInstanceFactory extends AsyncConstructor {
  ...
  constructor(deployer?: HardhatEthersSigner, proxyAdminLogic?: EcoProxyAdmin) {
    super(async () => {
      ...
      this.proxyAdminFactory = (
        await hre.ethers.getContractFactory("EcoProxyAdmin")
      ).connect(this.deployer);

      this.proxyAdminLogic = proxyAdminLogic
        ? proxyAdminLogic?.connect(deployer)
        : (await this.proxyAdminFactory.deploy(this.deployer)).connect(
            this.deployer,
          );
      this.EcoTUPFactory = (
        await hre.ethers.getContractFactory("EcoTUPWithAdminLogic")
      ).connect(this.deployer);
    });
  }

  async deployWithLogic<CT extends BaseContract>(
    logic: CT,
    input: BytesLike,
  ): Promise<EcoProxiedInstance<CT>> {
    const proxy = await this.EcoTUPFactory.deploy(
      this.proxyAdminLogic,
      logic,
      input,
    );
    ...
    inst.ecoProxy = proxy;
    inst.ecoProxyAdmin = this.proxyAdminFactory.attach(
      await this.getAdminAddress(inst),
    ) as EcoProxyAdmin;
    inst.ecoLogic = logic;
...
async function test() {
  ...
  const input = logic.interface.encodeFunctionData("initEcoERC20", [
    logic.runner,
    "name",
    "symbol",
    18n,
  ...
  const instByFactory = await EcoTUPFactory.deployWithFactory(
    logicFactory,
    [],
    input,
  );
  const instByAttachLogic = await EcoTUPFactory.attachWithLogic(
    logic,
    instByLogic,
  );
  const instByAttachFactory = await EcoTUPFactory.attachWithFactory(
    logicFactory,
    instByLogic,
  );

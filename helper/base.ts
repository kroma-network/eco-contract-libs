...
import { BaseContract, Signer, BytesLike, Addressable, ContractFactory, ZeroAddress, AddressLike } from "ethers";
...
export class ProxyFactory extends ContractInstance<EcoProxyAdmin__factory, EcoProxyAdmin> {
  ...
  async deployWithLogic<CT extends BaseContract>(logic: CT, input = "0x") {
...
async function testing() {
  const factory = await hre.ethers.getContractFactory("L2KGHManager");
  const test1 = await new ContractInstance("test", L2KGHManager__factory);
...
> extends ContractInstance<CF, CT> {
  ...
  async assign(info: {
    ...
    this.inst = info.inst;
    this.address = await this.inst.getAddress();
  }

  async _deployLogic(...deployArgs: unknown[]) { // TODO: Should Use Logic, deployLogic should deploy forced
    this.logic = await this.factory.deploy(...deployArgs);
    await this.logic.waitForDeployment();
  }

  async deployLogic(...deployArgs: unknown[]) { // TODO: Should Use Logic, deployLogic should deploy forced
    if (!this.logic) {
      await this._deployLogic(deployArgs);
    }
  }

  async deployWithInputBuilder(inputBuilder: () => Promise<string>) {
    await this.deployLogic();
    await this._deploy(await inputBuilder());
  }

  async useWithInputBuilder(inputBuilder: () => Promise<string>) {
    if (!this.inst) {
      await this.deployWithInputBuilder(inputBuilder);
      return true;
    }
    return false;
  }
  ...
  async deploy(input?: string) {
    await this.deployLogic();
    await this._deploy(input);
  }
  async _deploy(input?: string) {
  ...
  async attach(address: AddressLike) {
    await this.assign({ ...(await this.proxyFactory.attachWithFactory(this.factory, address)) });
  }
  ...
  async upgrade(address?: string) {
    ...
    if (!address) {
      await this._deployLogic();
      address = await this.logic.getAddress();
      console.log("in upgrade:", address);
    }
    await (await this.admin.upgradeAndCall(await this.proxy.getAddress(), address, "0x")).wait();
...
async function testing2() {
  const kgh = await new ProxiedInstance("launch", L2KGHManager__factory);
}

...
export class UUPSFactory extends AsyncConstructor {
  private static instance: UUPSFactory;
  ...
  constructor() {
    super(async () => {
      ...
      this.deployer = (await hre.ethers.getSigners())[0];
      this.factoryERC1967Proxy = (await hre.ethers.getContractFactory("ERC1967Proxy")).connect(this.deployer);
    });
  }

  public static async getInstance(): Promise<UUPSFactory> {
    if (!UUPSFactory.instance) {
      UUPSFactory.instance = await new UUPSFactory();
    }
    return UUPSFactory.instance;
  }

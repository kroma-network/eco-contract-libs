...
export type EcoContractFactory<CT extends BaseContract> =
...
export class ProxyInstanceFactory extends AsyncConstructor {
  ...
  async load(proxyAdminLogic?: EcoProxyAdmin) {
    ...
    if (proxyAdminLogic !== undefined) {
      ...
      reuseLogicAddress !== undefined &&
      reuseLogicAddress !== hre.ethers.ZeroAddress
    ) {
...
async function test() {
  ...
  const input = logic.interface.encodeFunctionData("initEcoERC20Mintable", [
    logic,
    "a",
    "a",
  ]);
